...
(function(){
  ...
  const { Test } = ChromeUtils.import("chrome://userscripts/content/000_test_runner.jsm");
  ...
  ).expect("000_test_runner.jsm,"+TEST_FILES.join(","));
  ...
  ).expect(TEST_FILES.length+";,test_module_script,test_utils");
  ...
  new Test(
    ...
    () => {
      return _ucUtils.writeFile("../userChrome.css","#mail-bar3{ background: #f00 !important; }")
    }
  ).expectError();
  ...
  new Test(
    ...
    () => {
      ...
      const getNavBarStyle = () => window.getComputedStyle(document.getElementById("mail-bar3"));
      ...
      return new Promise((resolve, reject) => {
        ...
        .then( () => {
          ...
          _ucUtils.writeFile("../userChrome.css","#mail-bar3{ background: #ba5 !important; }")
  ...
  ).expectAsync(42) // 42 bytes written
